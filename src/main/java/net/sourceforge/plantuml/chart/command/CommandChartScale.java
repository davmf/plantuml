...
public class CommandChartScale extends SingleLineCommand2<ChartDiagram> {

	public CommandChartScale() {
		super(false, getRegexConcat());
	}

	static IRegex getRegexConcat() {
		return RegexConcat.build(CommandChartScale.class.getName(), RegexLeaf.start(), //
				new RegexLeaf(1, "AXIS", "([hv])"), //
				new RegexLeaf("-axis"), //
	...
	protected CommandExecutionResult executeArg(ChartDiagram diagram, LineLocation location, RegexResult arg,
		...
		final String scaleStr = arg.get("SCALE", 0);

		try {
			final double scale = Double.parseDouble(scaleStr);
			if (scale <= 0) {
				return CommandExecutionResult.error("Scale must be positive: " + scaleStr);
			}
